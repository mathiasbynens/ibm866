...
import * as ibm866 from '../ibm866.mjs';
...
assert.deepStrictEqual(
	ibm866.encode('\uFFFF', { mode: 'replacement' }),
...
assert.deepStrictEqual(
	ibm866.decode(new Uint16Array([0xFFFF]), { mode: 'replacement' }),
	'\uFFFD',
...
assert.throws(
	() => {
		ibm866.decode(new Uint16Array([0xFFFF]), { mode: 'fatal' });
	},
	Error,
...
console.log('Testing `ibm866.labels`…');
assert.ok(Array.isArray(ibm866.labels));
assert.ok(ibm866.labels.length > 0);
